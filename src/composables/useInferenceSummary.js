...
// mewly useInferenceSummary 이식 — 분석 상태 탭의 데이터 계층. 게이트웨이 /summary
// (시간 버킷별 추론 카운트)를 읽어 그날의 자세 점유율과 개체 기준선을 계산한다.
// 해석은 주야 이원 체계다: 주간은 자세 3종(눕기/앉기/서기)의 점유율, 야간은 IR에서
// 앉음↔서기 혼동이 있어 누움/비누움 2단계로만 해석하고 "야간 뒤척임"(비누움 비율)을
// 지표로 삼는다. 주야 구분은 로컬 설정(analysisConfig.dayRange)이며, 정규화 분모는
// 라벨이 붙은 표본이다.

export const METRIC_KEYS = ['lying', 'sitting', 'standing', 'restless']

// 백엔드의 간헐 500을 흡수하기 위해 짧게 재시도한다.
...
// 한 추론이 여러 라벨에 매치될 수 있어 Σcounts는 근사 분모다 —
// 상태 간 상대 분포 목적에는 충분하다.
...
// 시간대 활동도: 라벨 표본 중 비누움(앉기+서기) 비율. 표본이 없으면 null.
export function hourActivity(h) {
...
// ── 리듬 판정 ──
// 지표별 표본이 minDays 미만이면 판정하지 않고(null), 편차 문턱은
// max(thresholdShare, sigmaMultiplier·σ)로 한다.
// 반환: null=기준선 부족 | []=이상 없음 | [{ label, direction: 'high'|'low' }]
