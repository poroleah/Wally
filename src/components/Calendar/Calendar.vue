<template>
  <div
    :class="[$style.calendar, warmDayMarkers ? $style.warmDayMarkers : '']"
  ...
  >
    <div :class="$style.monthYearLine">
      <img :class="$style.chevronLeftIcon" src="/icons/Calendar/chevron.svg" alt="" @click="prevMonth" />
      <div :class="$style.month">{{ currentMonth }}월</div>
      <img :class="[$style.chevronLeftIcon, $style.chevronRight]" src="/icons/Calendar/chevron.svg" alt="" @click="nextMonth" />
    </div>
...
function getDateKey(day) {
  if (day === null) return ''
  return dateObjectToKey({ year: currentYear.value, month: currentMonth.value, day })
}
...
<style module>
.calendar {
  --calendar-day-marker: var(--calendar-surface-soft);
  width: 100%;
...
.warmDayMarkers {
  --calendar-day-marker: var(--color-warm-neutral);
}
.monthYearLine {
  align-self: stretch;
  display: flex;
...
.div23 {
  ...
  height: 3.2rem;
  border-radius: 50%;
  background-color: var(--calendar-day-marker);
  display: flex;
  align-items: center;
  justify-content: center;
  font-size: 1.518rem;
...
.logBoneIcon {
  ...
  overflow: hidden;
  background-color: var(--calendar-day-marker);
  -webkit-mask: url('/icons/Calendar/Bone_Fill.svg') center / contain no-repeat;
  mask: url('/icons/Calendar/Bone_Fill.svg') center / contain no-repeat;
  pointer-events: none;
  z-index: 1;
  transform: translateY(0.125rem) scaleX(1.26) scaleY(1.3);
  transition: transform 0.18s ease;
}
.selectedLogDate::before {
  content: '';
  position: absolute;
  inset: 0;
  z-index: 0;
  background-color: var(--calendar-day-marker);
  -webkit-mask: url('/icons/Calendar/Bone_Selected.svg?v=4') center / contain no-repeat;
  mask: url('/icons/Calendar/Bone_Selected.svg?v=4') center / contain no-repeat;
  transform: translateY(0.125rem) scaleX(1.26) scaleY(1.3);
  pointer-events: none;
}
.selectedLogDate .logBoneIcon {
  transform: translateY(0.125rem) scaleX(1.26) scaleY(1.3);
}
.logDateNumber {
  z-index: 2;
...
.selected::after {
  ...
  inset: 0;
  z-index: 2;
  border: 1px solid var(--calendar-day-marker);
  border-radius: 50%;
  box-sizing: border-box;
  pointer-events: none;
}
